...
public class TClasificador {
	...
	private int[] ordenarPorQuicksort(int[] datosParaClasificar) {
		...
		int[] contador = {0};
        ordenarPorQuicksort(datosParaClasificar, i, j, contador);
		System.out.println("Contador: " + contador[0]);
		return datosParaClasificar;
    }

    private void ordenarPorQuicksort(int[] V, int i, int j, int[] contador) {
        if (i < j) { // Verificar que i sea menor que j para evitar bucle infinito
			contador[0]++;
            int indicePivote = encuentraPivote2(i, j, V);
            int pivote = V[indicePivote];
            int k = particion(i, j, pivote, V);
            ordenarPorQuicksort(V, i, k - 1, contador);
            ordenarPorQuicksort(V, k, j, contador);
        }
    }
